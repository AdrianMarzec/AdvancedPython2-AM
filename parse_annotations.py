...
# Funkcja do parsowania adnotacji z pliku XML do formatu CSV oraz YOLO
def parse_annotations(xml_path, output_csv="annotations.csv", output_yolo_dir="yolo_labels", image_dir="images"):
    ...
    # Folder na etykiety w formacie YOLO
    ...
    with open(output_csv, mode='w', newline='') as csv_file:
        ...
        for image in root.findall('image'):
            ...
            # Boxy opisujące obiekty
            for box in image.findall('box'):
                label = box.attrib['label']
                if label != "plate":
                    continue

                # Koordynaty boxa (plate)
...
# Funkcja dzieląca dane na zbiory treningowy i walidacyjny w strukturze YOLO
def split_yolo_dataset(image_dir='photos', label_dir='yolo_labels', output_dir='dataset', train_ratio=0.7):
    ...
    # Wyliczenie do podziału
    train_count = int(len(image_files) * train_ratio)
    train_files = image_files[:train_count]
    val_files = image_files[train_count:]

    # Rozdział do odpowiednich folderów
